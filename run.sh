...
gcc_versions=${USE_GCC_VERSIONS:-9.3.0}
...
do
    ...
    build_package=1 disable_cxx11_abi=0 ${this_dir}/build.sh /opt/vesoft/third-party/$version
    if [[ $arch = 'x86_64' ]]
    then
        rm -rf ./build
        rm -rf /opt/vesoft/third-party
        build_package=1 disable_cxx11_abi=1 ${this_dir}/build.sh /opt/vesoft/third-party/$version
    fi
